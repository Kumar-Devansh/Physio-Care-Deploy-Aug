...
For reliable inbox placement, use a transactional provider such as Amazon SES, Brevo, Mailgun, Postmark, or Resend with a clinic-owned sending domain. In that provider's dashboard, verify the domain and publish every DNS record it provides for **SPF** and **DKIM**. Then add a **DMARC** record, initially in monitoring mode:

```dns
_dmarc.your-domain.com  TXT  "v=DMARC1; p=none; rua=mailto:dmarc@your-domain.com"
```

Use the same verified domain in `DEFAULT_FROM_EMAIL`, `SERVER_EMAIL`, `CONTACT_EMAIL`, and `EMAIL_REPLY_TO`. Set `SITE_URL` to the public HTTPS site URL so email buttons never point to localhost or an IP address. These records and a consistent From domain are what mailbox providers use to trust the mail; changing the template alone cannot fix spam placement.

Gmail SMTP is suitable for local testing, but it is not recommended for production application mail. Gmail requires a Google App Password when two-step verification is enabled; do not put a normal Google password in `.env`.
