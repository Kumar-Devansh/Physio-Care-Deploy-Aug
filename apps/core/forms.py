...
class AppointmentForm(StyledFormMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["doctor"].label_from_instance = (
            lambda doctor: f"{doctor} — {doctor.specialization}"
        )
        self.fields["scheduled_at"].input_formats = ["%Y-%m-%dT%H:%M"]
        self.fields["mode"].choices = [("clinic", "In-clinic")]
        self.fields["reminder_channel"].widget = forms.Select(
...
class DoctorPrescriptionForm(DoctorPatientFormMixin, StyledFormMixin, forms.ModelForm):
    medicine_entries = forms.CharField(required=False, widget=forms.HiddenInput())

    class Meta:
        model = Prescription
        fields = ("patient", "diagnosis", "instructions", "issued_on")
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        entries = self.instance.medicine_entries if self.instance.pk else []
        self.initial.setdefault("medicine_entries", json.dumps(entries))

    def clean_medicine_entries(self):
        raw_entries = self.cleaned_data.get("medicine_entries", "[]")
        try:
            entries = json.loads(raw_entries or "[]")
        except json.JSONDecodeError as exc:
            raise forms.ValidationError("Add at least one complete medicine entry.") from exc
        required = {"name", "dosage", "frequency", "duration", "timing"}
        if not entries and self.data.get("medicines"):
            entries = [
                {
                    "name": line.strip(),
                ...
                for line in self.data["medicines"].splitlines()
                if line.strip()
            ]
        if not isinstance(entries, list) or not entries:
            raise forms.ValidationError("Add at least one medicine.")
        if any(not isinstance(entry, dict) or not required.issubset(entry) or not all(str(entry[key]).strip() for key in required) for entry in entries):
            raise forms.ValidationError("Complete every medicine field before saving.")
        return entries

    def save(self, commit=True):
        prescription = super().save(commit=False)
        entries = self.cleaned_data["medicine_entries"]
        prescription.medicine_entries = entries
        prescription.medicines = "\n".join(
            f"{entry['name']} — {entry['dosage']}, {entry['frequency']} for {entry['duration']} days, {entry['timing']}"
