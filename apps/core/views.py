...
@login_required
@require_POST
def notify_patient_of_care_updates(request, pk):
    doctor = _require_doctor(request)
    if not doctor:
        return redirect("dashboard")
    patient = get_object_or_404(_doctor_patients(doctor), pk=pk)
    title = "New care updates are available"
    message = f"{doctor} has added updates to your care plan. Open your patient portal to review them."
    Notification.objects.create(
        user=patient.user,
        title=title,
        message=message,
    ...
    send_user_notification_email(
        ...
        event_key=f"patient-care-update:{doctor.pk}:{patient.pk}:{uuid.uuid4()}",
    )
    messages.success(request, f"One care-update notification was sent to {patient}.")
...
def doctor_action(request, action):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        saves_for_final_notification = action in {"prescription", "exercise", "treatment"}
        ...
        if saves_for_final_notification:
            messages.success(
                request,
                f"{title} saved. Use Notify Patient on the patient timeline when you are ready.",
            )
            return redirect("doctor_patient_detail", pk=patient.pk)
    ...
    return render(
        ...
        "doctor/action_form.html",
        {
            ...
            "action_description": description,
            "action_icon": icon,
            "exercise_catalog": Exercise.objects.all() if action == "exercise" else None,
        },
    )
